...
Usage:
    python validate_pr.py [--quick] [--lint-only] [--test-only] [--cross-platform-only] [--modern]

Options:
    --quick              Skip slower tests for faster validation
    --lint-only          Only run linting checks (legacy: pylint)
    --test-only          Only run unit tests
    --cross-platform-only Only run cross-platform compatibility test
    --modern             Use modern development tools (ruff, mypy, pytest-cov)
...
class ValidationRunner:
    ...
    def run_command(self, cmd, description, cwd=None, check_return_code=True, env=None):
        ...
        print(
            f"{Colors.CYAN}Running: {' '.join(cmd) if isinstance(cmd, list) else cmd}{Colors.END}"
        )
    ...
    def validate_python_syntax(self):
        ...
        for py_file in python_files:
            try:
                with open(self.repo_root / py_file, encoding="utf-8") as f:
    ...
    def run_cross_platform_test(self):
        ...
        if not result:
            ...
            if "numpy" in proc_result.stderr and "architecture" in proc_result.stderr:
                self.print_warning(
                    "Cross-platform test failed due to numpy architecture (expected locally)"
                )
    ...
    def print_summary(self):
        ...
        if self.failed_checks:
            ...
            print(
                f"{Colors.RED}Please fix the issues above before submitting your PR.{Colors.END}"
            )
        else:
...
def main():
    ...
    print(
        f"{Colors.CYAN}This tool runs all CI/CD checks locally to validate your PR{Colors.END}"
    )
