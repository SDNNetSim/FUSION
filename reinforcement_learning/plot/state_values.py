...
def _plot_matrix(axes, color_matrix, n_nodes, t_label, algo, path_colors):
    """
    ...
    axes.set_title(f"Best Path Matrix (Algorithm: {algo}, Traffic: {t_label})")
...
def _plot_bar_chart(pair_dict, t_label, algo):
    ...
    plt.bar(np.arange(max_paths), path_counts)
    plt.xticks(np.arange(max_paths), [f"Path {i}" for i in range(max_paths)])
    plt.xlabel("Path Index")
    plt.ylabel("Count")
    plt.title(f"Best Path Count (Algorithm: {algo}, Traffic: {t_label})")
    plt.tight_layout()
    plt.show()
...
def plot_best_path_matrix(averaged_state_values_by_volume, path_colors=None):
    ...
    for algo, volumes_dict in averaged_state_values_by_volume.items():
        for t_label, pair_dict in volumes_dict.items():
            ...
            _plot_matrix(ax, color_matrix, n_nodes, t_label, algo, path_colors)
            max_paths = max((len(vals) for vals in pair_dict.values()), default=0)
            patches = []
            ...
            ax.legend(handles=patches, title="Path Colors", bbox_to_anchor=(1.05, 1), loc="upper left")
            fig.tight_layout()
            plt.show()
