...
class TestSimStats(unittest.TestCase):
    ...
    def test_end_iter_update(self):
        ...
        expected_trans_mean = 300 / (
            self.engine_props["num_requests"] - self.sim_stats.blocked_requests
        )
    ...
    def test_get_snapshot_info(self):
        ...
        network_spectrum_dict = {
            (0, 1): {
                "cores_matrix": [np.array([0, 1, 0, -1]), np.array([1, 0, -1, 0])]
            },
            (1, 0): {
                "cores_matrix": [np.array([0, -1, 1, 0]), np.array([-1, 1, 0, 0])]
            },
        }
        path_list = [(0, 1)]
    ...
    def test_update_snapshot(self):
        ...
        self.assertEqual(
            snapshot["num_segments"][0], self.sim_stats.current_transponders
        )
    ...
    def test_init_mods_weights_bws(self):
        ...
        for bandwidth, mod_obj in self.sim_stats.engine_props["mod_per_bw"].items():
            ...
            for modulation in mod_obj.keys():
                self.assertIn(
                    modulation,
                    self.sim_stats.stats_props.modulations_used_dict[bandwidth],
                )
                self.assertIn(
                    modulation, self.sim_stats.stats_props.weights_dict[bandwidth]
                )
                self.assertEqual(
                    self.sim_stats.stats_props.modulations_used_dict[bandwidth][
                        modulation
                ...
                self.assertEqual(
                    self.sim_stats.stats_props.weights_dict[bandwidth][modulation], []
                )
    ...
    def test_init_stat_dicts(self):
        ...
        expected_block_reasons_dict = {
            "distance": 0,
            "congestion": 0,
            "xt_threshold": 0,
        }
        self.assertEqual(
            self.sim_stats.stats_props.block_reasons_dict, expected_block_reasons_dict
        )
    ...
    def test_handle_iter_lists(self):
        ...
        self.assertEqual(
            self.sim_stats.stats_props.modulations_used_dict["50GHz"]["QPSK"], 1
        )
        self.assertEqual(
            self.sim_stats.stats_props.modulations_used_dict["75GHz"]["16QAM"], 1
        )
    ...
    def test_iter_update(self):
        ...
        self.sim_stats.stats_props.block_reasons_dict = {
            "congestion": 0,
            "distance": 0,
            "xt_threshold": 0,
        }
        ...
        self.sim_stats.iter_update(
            req_data=req_data_blocked,
            sdn_data=sdn_data_blocked,
            network_spectrum_dict={},
        )
        ...
        self.sim_stats.iter_update(
            req_data=req_data_routed,
            sdn_data=sdn_data_routed,
            network_spectrum_dict=dummy_net_spec,
        )
    ...
    def test_get_iter_means(self):
        ...
        self.assertEqual(
            self.sim_stats.stats_props.weights_dict["50GHz"]["QPSK"], expected_mod_obj
        )
        self.assertEqual(
            self.sim_stats.stats_props.snapshots_dict[1]["occupied_slots"], 20
        )
