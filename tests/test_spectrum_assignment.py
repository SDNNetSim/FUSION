...
class RouteProps:  # pylint: disable=too-few-public-methods
    ...
    def __init__(
        ...
        path_index=0,
        input_power=0.001,
        freq_spacing=12500000000.0,
        mci_worst=6.334975555658596e-27,
        max_link_length=None,
        span_length=100.0,
        max_span=None,
    ):
        self.paths_matrix = paths_matrix if paths_matrix is not None else []
        self.modulation_formats_matrix = (
            mod_formats_matrix if mod_formats_matrix is not None else []
        )
...
class TestSpectrumAssignment(unittest.TestCase):
    ...
    def test_allocate_best_fit(self):
        ...
        self.spec_assign.spectrum_helpers.check_other_links = MagicMock(
            return_value=True
        )
    ...
    def test_setup_first_last(self):
        ...
        self.assertEqual(
            core_list,
            list(range(0, self.spec_assign.engine_props_dict["cores_per_link"])),
        )
    ...
    def test_get_spectrum(self):
        ...
        with (
            ...
            self.assertTrue(
                mock_get_spectrum.called,
                "Expected '_determine_spectrum_allocation' to be called.",
            )
            self.assertEqual(
                mock_get_spectrum.call_count,
                1,
                "Expected '_determine_spectrum_allocation' to be called once.",
            )

            # Check that the spectrum was successfully allocated
            self.assertTrue(
                self.spec_assign.spectrum_props.is_free,
                "Expected spectrum allocation to be successful (is_free=True).",
            )
            self.assertEqual(
                self.spec_assign.spectrum_props.modulation,
                "QPSK",
