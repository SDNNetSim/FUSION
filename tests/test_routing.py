...
class TestRouting(unittest.TestCase):
    ...
    def test_find_least_cong_path(self):
        ...
        self.assertEqual(
            self.instance.route_props.paths_matrix[0]["path_list"], ["A", "C"]
        )
    ...
    def test_least_xt_cost_path_selection(self):
        ...
        self.assertEqual(
            selected_path,
            expected_path,
            f"Expected path {expected_path} but got {selected_path}",
        )
    ...
    def test_least_weight_path_selection(self):
        ...
        try:
            with (
                patch(
                    "fusion.core.routing.sort_nested_dict_vals",
                    return_value={
                        "QPSK": {"max_length": 10},
    ...
    def test_find_k_shortest_paths(self):
        ...
        self.assertEqual(
            len(self.instance.route_props.paths_matrix),
        ...
        for mod_format_list in self.instance.route_props.modulation_formats_matrix:
            self.assertEqual(
                len(mod_format_list),
                1,
                "Each path should have exactly one modulation format",
            )
        for weight in self.instance.route_props.weights_list:
            self.assertIsInstance(
                weight, (int, float), "Each weight should be a number"
            )
    ...
    def test_find_least_nli(self):
        ...
        """
        self.engine_props["mod_per_bw"] = {"50GHz": {"QPSK": {"slots_needed": 10}}}
        ...
        try:
            with (
                patch.object(
                    self.instance.route_help_obj, "get_nli_cost", return_value=1.0
                ),
                patch(
                    "fusion.core.routing.sort_nested_dict_vals",
                    return_value={
                ...
                for link_tuple in list(
                    self.instance.sdn_props.network_spectrum_dict.keys()
                )[::2]:
                    source, destination = link_tuple
                    self.assertIn(
                        "nli_cost",
                        self.instance.sdn_props.topology[source][destination],
    ...
    def test_find_least_xt(self):
        ...
        with (
            patch("fusion.sim.utils.find_free_slots", return_value={"free_slots": []}),
            patch.object(
                self.instance.route_help_obj, "find_xt_link_cost", return_value=0.1
            ),
            patch.object(
                self.instance.route_help_obj, "get_max_link_length", return_value=100
            ),
        ):
            ...
            for link_list in list(self.sdn_props.network_spectrum_dict.keys())[::2]:
                ...
                self.assertIn(
                    "xt_cost",
                    self.sdn_props.topology[source][destination],
                    "XT cost not set for link",
                )
    ...
    def test_load_k_shortest_success(self):
        ...
        with patch(
            "fusion.sim.utils.sort_nested_dict_vals",
            return_value={
                "64-QAM": {"max_length": 30},
                "16-QAM": {"max_length": 20},
                "QPSK": {"max_length": 10},
            ...
            with patch(
                "numpy.load", return_value=self.route_props.loaded_data_dict_mock
            ):
                ...
                self.assertEqual(
                    self.instance.route_props.weights_list[:1], expected_weights
                )

                # Verify modulation formats were added in reverse order
                expected_mod_formats = ["64-QAM", "16-QAM", "QPSK"]
                self.assertGreater(
                    len(self.instance.route_props.modulation_formats_matrix[0]), 0
                )
                self.assertEqual(
