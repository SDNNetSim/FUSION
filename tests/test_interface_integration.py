...
class TestInterfaceIntegration(unittest.TestCase):
            ...
            for band in ["c", "l", "s"]:
                ...
                for core_num in range(7):  # 7 cores per link
                    ...
                    if core_num == 0:
                        core_array[100:110] = [
                            1001
                        ] * 10  # Request 1001 occupies slots 100-109
                        core_array[200:205] = [
                            1002
                        ] * 5  # Request 1002 occupies slots 200-204
    ...
    def test_complete_request_processing(self):
        ...
        self.assertTrue(
            hasattr(pipeline, "process_request"),
            "Pipeline missing process_request method",
        )
            ...
            with self.subTest(routing=routing, spectrum=spectrum, snr=snr):
                try:
                    ...
                    self.assertIsInstance(
                        pipeline.routing_algorithm.algorithm_name, str
                    )
                    self.assertIsInstance(
                        pipeline.spectrum_algorithm.supports_multiband, bool
                    )
                    self.assertIsInstance(
                        pipeline.snr_algorithm.supports_multicore, bool
                    )

                except (ValueError, NotImplementedError):
                    self.skipTest(
                        f"Algorithm combination not yet implemented: {routing}, {spectrum}, {snr}"
                    )
