...
class StatsPersistence:
    ...
    def save_stats(
            ...
            stats_props: Any,
            blocking_stats: Dict[str, Optional[float]],
    ...
    def _prepare_iteration_stats(self, stats_props: Any, iteration: int = 0) -> Dict[str, Any]:  # pylint: disable=unused-argument
        """
        ...
        for stat_key in vars(stats_props).keys():
            if stat_key in ('transponders_list', 'hops_list', 'lengths_list', 'route_times_list', 'crosstalk_list'):
                # Map new property names to old output key names for backward compatibility
                ...
                if stat_key in ['start_slot_list', 'end_slot_list'] and not self.engine_props.get(
    ...
    def _get_save_path(self, base_file_path: Optional[str] = None) -> str:
        """
