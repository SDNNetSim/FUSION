...
# Dual-path spectrum reservation for 1+1 protection
...
def find_available_slots_on_path(
    ...
    if "cores_matrix" not in link_spectrum:
        return set()
    ...
    for start in range(len(spectrum_array) - slots_needed + 1):
        ...
        if all(spectrum_array[start + i] == 0.0 for i in range(slots_needed)):
    ...
    for i in range(1, len(path) - 1):
        ...
        available_starts = {
            ...
            if all(spectrum_array[start + j] == 0.0 for j in range(slots_needed))
        }
...
def allocate_spectrum_on_path(
    ...
    for i in range(len(path) - 1):
        link = (path[i], path[i + 1])
        rev_link = (path[i + 1], path[i])

        if link not in network_spectrum_dict:
            continue

        # Allocate on forward link
        spectrum_array = network_spectrum_dict[link]["cores_matrix"][band][core]
        for slot_idx in range(start_slot, end_slot):
            if slot_idx < len(spectrum_array):
                spectrum_array[slot_idx] = float(request_id)

        # Allocate on reverse link
        if rev_link in network_spectrum_dict:
            rev_spectrum_array = network_spectrum_dict[rev_link]["cores_matrix"][band][
                core
            ]
            for slot_idx in range(start_slot, end_slot):
                if slot_idx < len(rev_spectrum_array):
                    rev_spectrum_array[slot_idx] = float(request_id)
...
def release_spectrum_on_path(
    ...
    :param network_spectrum_dict: Network spectrum state
    :type network_spectrum_dict: dict[tuple[Any, Any], dict[str, Any]]
    :param path: List as list of node IDs
    :type path: list[int]
    :param start_slot: Starting slot index
    :type start_slot: int
    :param end_slot: Ending slot index (exclusive)
    ...
    for i in range(len(path) - 1):
        ...
        # Release on forward link
        spectrum_array = network_spectrum_dict[link]["cores_matrix"][band][core]
        for slot_idx in range(start_slot, end_slot):
            if slot_idx < len(spectrum_array):
                spectrum_array[slot_idx] = 0.0

        # Release on reverse link
        if rev_link in network_spectrum_dict:
            rev_spectrum_array = network_spectrum_dict[rev_link]["cores_matrix"][band][
                core
            ]
            for slot_idx in range(start_slot, end_slot):
                if slot_idx < len(rev_spectrum_array):
                    rev_spectrum_array[slot_idx] = 0.0
