...
class SpectrumAssignment:
    ...
    def __init__(
        self, engine_props: dict, sdn_props: SDNProps, route_props: RoutingProps
    ):
    ...
    def _setup_first_last_allocation(self):
        ...
        for current_core_number in core_numbers_list:
            cores_spectrum_matrix.append(
                [
                    self.spectrum_props.cores_matrix[band][current_core_number]
    ...
    def get_spectrum_dynamic_slicing(
        self,
        _mod_format_list: list,
        _slice_bandwidth: str = None,
        path_index: int = None,
    ):
        """
        Controls the class, attempts to find an available spectrum.
