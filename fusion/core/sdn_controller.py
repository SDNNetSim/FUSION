...
class SDNController:
    ...
    def __init__(self, engine_props: Dict[str, Any]) -> None:
        ...
        self.slicing_manager = LightPathSlicingManager(engine_props=self.engine_props,
    ...
    def release(self) -> None:
        ...
        for source, dest in zip(self.sdn_props.path_list, self.sdn_props.path_list[1:]):
            for band in self.engine_props['band_list']:
                for core_num in range(self.engine_props['cores_per_link']):
                    ...
                    guard_band_indices = np.where(core_arr == (self.sdn_props.request_id * -1))

                    for req_index in request_id_indices:
                        self.sdn_props.network_spectrum_dict[(source, dest)]['cores_matrix'][band][core_num][
                            req_index] = 0
                        self.sdn_props.network_spectrum_dict[(dest, source)]['cores_matrix'][band][core_num][
                            req_index] = 0
        ...
        try:
            ...
            logger.warning("Throughput update skipped due to missing or invalid timing/bandwidth: %s", e)

    def _allocate_gb(self, band: str, core_matrix: List[Any], rev_core_matrix: List[Any],
                     core_num: int, end_slot: int) -> None:
    ...
    def allocate(self) -> None:
        ...
        for link_tuple in zip(self.sdn_props.path_list, self.sdn_props.path_list[1:]):
            ...
            reverse_spectrum_slots_set = set(rev_link_dict['cores_matrix'][band][core_num][start_slot:end_slot])
    ...
    def _handle_slicing_request(self, path_list: List[Any], path_index: int,
                                forced_segments: int, force_slicing: bool) -> bool:  # pylint: disable=unused-argument
        ...
        if self.engine_props['dynamic_lps']:
            return self.slicing_manager.handle_dynamic_slicing_direct(
                path_list=path_list, path_index=path_index, forced_segments=forced_segments,
                sdn_controller=self
            )
    ...
    def _setup_routing(self, force_route_matrix: Optional[List[Any]],
                       force_mod_format: Optional[str]) -> tuple[List[Any], float]:
    ...
    def _get_ml_prediction(self, ml_model: Optional[Any], request_dict: Dict[str, Any]) -> float:
    ...
    def _process_single_path(self, path_list: List[Any], path_index: int,
                             mod_format_list: List[str], forced_segments: float,
                             force_slicing: bool, segment_slicing: bool,
                             forced_index: Optional[int], force_core: Optional[int],
                             forced_band: Optional[str]) -> bool:
    ...
    def _finalize_successful_allocation(self, path_index: int, route_time: float,
                                        force_slicing: bool, segment_slicing: bool) -> None:
    ...
    def handle_event(self, request_dict: Dict[str, Any], request_type: str, force_slicing: bool = False,
                     force_route_matrix: Optional[List[Any]] = None, forced_index: Optional[int] = None,
                     force_core: Optional[int] = None, ml_model: Optional[Any] = None,
                     force_mod_format: Optional[str] = None, forced_band: Optional[str] = None) -> None:
            ...
            for path_index, path_list in enumerate(route_matrix):
                if path_list is not False:
                    ...
                    success = self._process_single_path(
                        path_list, path_index, mod_format_list, forced_segments,
                        force_slicing, segment_slicing, forced_index,
                        force_core, forced_band
                    )

                    if success:
                        self._finalize_successful_allocation(path_index, route_time,
                                                             force_slicing, segment_slicing)
                        return

            # Try segment slicing if not already tried
            if (self.engine_props['max_segments'] > 1 and
                    self.sdn_props.bandwidth != '25' and not segment_slicing):
    ...
    def _allocate_slicing(self, num_segments: int, mod_format: str, path_list: List[Any],
                          bandwidth: str) -> None:
        ...
        self.slicing_manager.allocate_slicing_direct(
            ...
            path_list=path_list, bandwidth=bandwidth, sdn_controller=self
        )

    def _handle_dynamic_slicing(self, path_list: List[Any], path_index: int,
