...
class Routing:
    ...
    def __init__(self, engine_props: Dict[str, Any], sdn_props: Any) -> None:
        ...
        self._current_algorithm: Optional[Any] = None

        logger.debug("Initialized routing coordinator with method: %s", engine_props.get('route_method'))
    ...
    def _handle_external_ksp(self) -> None:
        ...
        try:
            ...
            logger.warning("External KSP loading failed: %s. Falling back to computed k-shortest paths.", e)
    ...
    def get_route(self) -> None:
        ...
        logger.info("Starting %s routing from %s to %s", route_method, self.sdn_props.source,
    ...
    def find_least_weight(self, weight: str) -> None:
        ...
        if weight == 'length':
            ...
            for path_list in paths:
                path_weight = find_path_len(path_list=path_list, topology=self.sdn_props.topology)
                ...
                try:
                    if hasattr(self.sdn_props, 'mod_formats') and self.sdn_props.mod_formats:
                        modulation_format = get_path_mod(self.sdn_props.mod_formats, path_weight)
                    else:
    ...
    def load_k_shortest(self) -> None:
        ...
        source_destination = [int(self.sdn_props.source), int(self.sdn_props.destination)]
