...
def generate_uniform_random_variable(
    ...
    :type scale_parameter: float | None
...
def generate_exponential_random_variable(scale_parameter: float) -> float:
    ...
    # Cast to float to satisfy mypy (np.log returns Any)
    return float((-1.0 / scale_parameter) * np.log(uniform_sample))
