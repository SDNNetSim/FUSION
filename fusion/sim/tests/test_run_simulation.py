...
class TestRunSimulation:
    ...
    def test_run_simulation_calls_batch_runner(
        self, mock_batch_runner: MagicMock
    ) -> None:
    ...
    @patch("fusion.sim.run_simulation.run_batch_simulation")
    def test_run_simulation_returns_none_when_no_results(
        self, mock_batch_runner: MagicMock
    ) -> None:
    ...
    @patch("fusion.sim.run_simulation.run_batch_simulation")
    def test_run_simulation_returns_first_result_for_compatibility(
        self, mock_batch_runner: MagicMock
    ) -> None:
...
class TestRunSimulationPipeline:
    ...
    def test_run_simulation_pipeline_loads_config_and_runs(
        ...
        mock_batch_runner.assert_called_once_with(
            {"s1": {"network": "test_net"}}, parallel=False, num_processes=None
        )
        assert result == [{"erlang": 100}]

    @patch("fusion.sim.run_simulation.run_batch_simulation")
    @patch("fusion.cli.config_setup.load_and_validate_config")
    def test_run_simulation_pipeline_handles_parallel_execution(
        self, mock_load_config: MagicMock, mock_batch_runner: MagicMock
    ) -> None:
        ...
        # Assert
        mock_batch_runner.assert_called_once_with(
            {"s1": {"network": "test_net"}}, parallel=True, num_processes=4
        )
    ...
    @patch("fusion.sim.run_simulation.run_batch_simulation")
    @patch("fusion.cli.config_setup.load_and_validate_config")
    def test_run_simulation_pipeline_ignores_stop_flag(
        self, mock_load_config: MagicMock, mock_batch_runner: MagicMock
    ) -> None:
