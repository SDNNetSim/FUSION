...
class PlotHelpers:  # pylint: disable=too-few-public-methods
    ...
    def _find_ai_stats(self, cores_per_link: int) -> None:
        ...
        if ai_dict is not None:
            # Type narrowing
            assert self.time is not None and self.sim_num is not None and self.plot_props.plot_dict is not None
            for ai_key in (
                "learn_rate",
                "discount_factor",
                "epsilon_start",
    ...
    def _get_data(self) -> None:
        assert self.file_info is not None  # Type narrowing
    ...
    def get_file_info(self, sims_info_dict: dict) -> None:
        """
        ...
        for network_list, dates_list, times_list in zip(networks_matrix, dates_matrix, times_matrix, strict=False):
            for (
                network,
                date,
                time,
...
def _check_filters(file_dict: dict, filter_dict: dict) -> bool:
