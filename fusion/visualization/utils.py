...
from fusion.sim.utils import dict_to_list, list_to_title, update_matrices
...
class PlotHelpers:  # pylint: disable=too-few-public-methods
    ...
    def _find_ai_stats(self, cores_per_link: int):
        ...
        for ai_key in (
            ...
            if ai_key in ("sum_rewards_dict", "sum_errors_dict"):
                ...
                self.plot_props.plot_dict[self.time][self.sim_num][label].append(
                    list(ai_dict[ai_key].values())
                )
            else:
                self.plot_props.plot_dict[self.time][self.sim_num][ai_key] = ai_dict[
                    ai_key
                ]
    ...
    def _find_misc_stats(self):
        ...
        times_list = (
            dict_to_list(self.erlang_dict["iter_stats"], "route_times_mean") * 10**3
        )
        ...
        self.plot_props.plot_dict[self.time][self.sim_num].lengths_list.append(
            average_length
        )
        self.plot_props.plot_dict[self.time][self.sim_num].hops_list.append(average_hop)
        self.plot_props.plot_dict[self.time][self.sim_num].times_list.append(
            average_time
        )
        self.plot_props.plot_dict[self.time][self.sim_num].cong_block_list.append(
            average_cong
        )
        self.plot_props.plot_dict[self.time][self.sim_num].dist_block_list.append(
            average_dist
        )
    ...
    def _find_snapshot_usage(self):
        ...
        self.plot_props.plot_dict[self.time][self.sim_num].active_req_matrix = np.mean(
            active_req_matrix, axis=0
        )
        self.plot_props.plot_dict[self.time][self.sim_num].block_req_matrix = np.mean(
            block_req_matrix, axis=0
        )
        self.plot_props.plot_dict[self.time][self.sim_num].occ_slot_matrix = np.mean(
            occ_slot_matrix, axis=0
        )
