...
for network in NETWORK_LIST:
    for arrival_rate in ARRIVAL_RATE_LIST:
        ...
        def read_files():
            ...
            with open(output_fp, encoding="utf-8") as file_path:
                try:
                    output_dict = json.load(file_path)
                except json.JSONDecodeError:
                    print("Output dict not found.")
                    return False, False
            with open(input_fp, encoding="utf-8") as file_path:
                try:
        ...
        def get_dict(
            input_dict: dict, output_dict: dict, network: str, arrival_rate: int
        ):
            """
