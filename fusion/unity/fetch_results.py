...
def convert_output_path_to_input_path(
    absolute_output_path: pathlib.PurePosixPath,
    ...
    if "output" not in path_parts_list:
        raise ValueError(
            f"Path does not contain 'output' directory: {absolute_output_path}"
        )
...
def get_last_path_segments(
    path: pathlib.PurePosixPath, segment_count: int
) -> pathlib.PurePosixPath:
    """
...
def extract_topology_from_output_path(
    output_directory_path: pathlib.PurePosixPath,
) -> str:
    """
    ...
    if "output" not in path_parts_list:
        raise ValueError(
            f"Path does not contain 'output' directory: {output_directory_path}"
        )

    output_index = path_parts_list.index("output")
    if output_index + 1 >= len(path_parts_list):
        raise ValueError(
            f"No topology directory found after 'output' in path: {output_directory_path}"
        )
...
def synchronize_remote_directory(
    ...
    absolute_directory_path: pathlib.PurePosixPath,
...
def synchronize_remote_file(
    remote_root_path: str,
    remote_file_path: pathlib.PurePosixPath,
...
def synchronize_simulation_logs(
    ...
    topology_name: str,
    date_timestamp_path: pathlib.PurePosixPath,
    destination_root_path: pathlib.Path,
    is_dry_run: bool,
) -> None:
    """
    ...
    """
    remote_logs_path = (
        pathlib.PurePosixPath(path_algorithm_name) / topology_name / date_timestamp_path
    )
    local_logs_path = (
...
def extract_path_algorithm_from_input_directory(
...
def iterate_runs_index_file(
    index_file_path: pathlib.Path,
) -> Iterator[pathlib.PurePosixPath]:
    """
...
def main() -> None:
    ...
    try:
        ...
        configuration_data = yaml.safe_load(
            configuration_file_path.read_text(encoding="utf-8")
        )
    ...
    for output_directory_path in iterate_runs_index_file(runs_index_local_path):
        ...
        path_algorithm = extract_path_algorithm_from_input_directory(
