...
@pytest.fixture
def k_path_cache(sample_topology):
...
def test_get_k_paths_returns_empty_for_disconnected():
...
def test_topology_must_have_nodes():
...
def test_find_free_blocks(k_path_cache):
...
def test_fragmentation_indicator_calculation(sample_topology):
...
def test_no_free_slots_sets_min_residual_to_zero(sample_topology):
...
def test_path_features_without_failure_manager(sample_topology):
...
def test_dist_to_disaster_centroid(sample_topology):
...
def test_cache_handles_reverse_link_direction(sample_topology):
...
def test_get_cache_size(k_path_cache):
...
def test_empty_spectrum_dict_handled_gracefully(sample_topology):
