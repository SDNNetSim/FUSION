...
class SpectrumRegistry:
    ...
    def get(self, name: str) -> Type[AbstractSpectrumAssigner]:
        ...
        if name not in self._algorithms:
            raise KeyError(f"Spectrum assignment algorithm '{name}' not found. "
                           f"Available algorithms: {list(self._algorithms.keys())}")

        return self._algorithms[name]

    def create(self, name: str, engine_props: dict, sdn_props: object,
...
def register_spectrum_algorithm(name: str, algorithm_class: Type[AbstractSpectrumAssigner]):
