...
class LastFitSpectrum(AbstractSpectrumAssigner):
    ...
    def _find_last_fit_on_path(self, core_list: List[int], band_list: List[str]) -> bool:
        ...
        for i in range(len(self.spectrum_props.path_list) - 1):
            ...
            if link_assignment and (best_assignment is None or
                                    link_assignment['start_slot'] > best_assignment['start_slot']):
        ...
        if best_assignment:
            ...
            self.spectrum_props.core_num = best_assignment['core_num']
            self.spectrum_props.curr_band = best_assignment['band']
            self.spectrum_props.is_free = True
            return True

        return False

    def _find_best_assignment_for_link(self, link_dict: Dict, core_list: List[int],
                                       band_list: List[str]) -> Optional[Dict]:
        ...
        for core_num in core_list:
            for band in band_list:
                ...
                if assignment and (best_assignment is None or
                                   assignment['start_slot'] > best_assignment['start_slot']):
                    best_assignment = assignment

        return best_assignment

    def _try_core_band_assignment(self, link_dict: Dict, core_num: int, band: str) -> Optional[Dict]:
        ...
        if self._verify_path_assignment(assignment['start_slot'],
                                        assignment['end_slot'],
                                        core_num, band):
            return assignment
    ...
    def _find_last_contiguous_block(self, core_array, core_num: int, band: str) -> Optional[Dict]:
        ...
        if len(free_slots) < self.spectrum_props.slots_needed:
            return None

        # Search from the end (highest indices) backwards
        for i in range(len(free_slots) - self.spectrum_props.slots_needed, -1, -1):
            ...
            if all(slot in free_slots for slot in required_slots):
                ...
                if is_contiguous:
                    return {
                        ...
                        'end_slot': start_slot + self.spectrum_props.slots_needed - 1,
                        'core_num': core_num,
                        'band': band
                    }
