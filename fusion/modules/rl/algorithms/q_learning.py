...
class QLearning:
    ...
    def get_max_future_q(
        ...
        new_cong, _ = (
            find_core_cong(core_index, network_spectrum_dict, path_list)
            if flag == "core"
            else find_path_cong(path_list, network_spectrum_dict)
        )
        new_cong_index = classify_cong(
    ...
    def _calc_q_averages(self, stats_flag: str, trial: int) -> None:
        ...
        """
        self.rewards_stats_dict = calc_matrix_stats(
            self.props.rewards_dict[stats_flag]["rewards"]
        )
        self.error_stats_dict = calc_matrix_stats(
