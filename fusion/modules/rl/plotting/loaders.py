        ...
        for run_dir in run_root.iterdir():
            ...
            if is_drl:
                ...
                for seed_dir in run_dir.glob("s*"):
                    ...
                    input_file_path = ROOT_INPUT_DIRECTORY / network / date / timestamp / f"sim_input_{s_num}.json"
                    ...
                    if input_file_path.exists():
                        ...
                        algo = f"{method}_{'inf' if k_paths > 4 else k_paths}" if method == "k_shortest_path" else method
...
def load_metric_for_runs(
    ...
    for date in dates:
        for simulation_directory in (ROOT_OUTPUT_DIRECTORY / network / date).rglob("s*"):
            if not simulation_directory.is_dir():
                continue

            if drl:
                result = _handle_drl_run(simulation_directory, run_ids, metric, network, date)
            else:
                result = _handle_non_drl_run(simulation_directory, run_ids, metric, network, date)
...
def _handle_drl_run(simulation_directory: Path, run_ids: Iterable[str], metric: str, network: str, date: str):  # pylint: disable=unused-argument
    base_run_directory = simulation_directory.parent
    seed = simulation_directory.name

    if metric == "sim_times":
        max_seed = max(int(p.name.lstrip("s")) for p in simulation_directory.parent.glob("s*"))
        if int(seed.lstrip("s")) != max_seed:
            return None

    metadata_file_path = next(base_run_directory.glob("s*/meta.json"), None)
    metadata_run_id: Optional[str] = None
    algo = "unknown"
    ...
    if run_ids and not any(rid in run_ids for rid in (unique_run_id, composite_run_id, metadata_run_id)):
        return None

    metric_values = _load_metric_jsons(simulation_directory, metric)
    _augment_with_logs(metric, drl=True, metric_vals=metric_values, algo=algo, network=network, base_run_dir=base_run_directory)

    stamp = f"{simulation_directory.parent.parent.name}_{simulation_directory.parent.name}" if metric == "sim_times" else None
    return unique_run_id, algo, metric_values, stamp


def _handle_non_drl_run(simulation_directory: Path, run_ids: Iterable[str], metric: str, network: str, date: str):  # pylint: disable=unused-argument
    ...
    input_file_path = ROOT_INPUT_DIRECTORY / network / date_string / parent_directory.name / f"sim_input_{seed}.json"
    algo = "unknown"
    if input_file_path.exists():
        input_data = _safe_load_json(input_file_path) or {}
        method = input_data.get("route_method")
...
def _augment_with_logs(metric: str, drl: bool, metric_vals: Dict[str, Any], algo: str,
    ...
    if metric == "memory" and drl:
        logs_file_path = ROOT_LOGS_DIRECTORY / algo / network / base_run_dir.parent.name / base_run_dir.name / "memory_usage.npy"
        if logs_file_path.exists():
            try:
                memory_array = np.load(logs_file_path)
                metric_vals["overall"] = float(memory_array.max() / (1024 ** 2))
            except (OSError, ValueError) as exc:
                raise DataLoadError(f"Failed to load memory usage data from {logs_file_path}: {exc}") from exc

    elif metric == "state_values" and drl:
        logs_directory = ROOT_LOGS_DIRECTORY / algo / network / base_run_dir.parent.name / base_run_dir.name
        if not logs_directory.is_dir():
            return

        state_values_regex = re.compile(r"state_vals_e(?P<erl>\d+\.?\d*)_.*?_t(?P<trial>\d+)\.json")
        ...
        for file_path in logs_directory.glob("state_vals_*.json"):
            ...
            try:
                with file_path.open("r", encoding="utf-8") as f:
                    ...
                    state_values_data = {ast.literal_eval(k): v for k, v in state_values_data.items()}
                    state_values[erlang][trial] = state_values_data
            except (OSError, json.JSONDecodeError, ValueError) as exc:
                raise DataLoadError(f"Failed to load state values from {file_path}: {exc}") from exc
