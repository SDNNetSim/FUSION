...
class HyperparamConfig:  # pylint: disable=too-few-public-methods
    ...
    def _softmax(self, q_vals_list: list[float]) -> np.ndarray[Any, Any]:
        """
        Compute the softmax probabilities for a given set of Q-values.

        Uses the current temperature parameter to control exploration vs
        exploitation.
        Higher temperature values lead to more uniform action
        probabilities.
        ...
        if self.temperature is None:
            raise ValueError("Temperature must be set before computing softmax")
        exp_values = np.exp(np.array(q_vals_list) / self.temperature)
        probabilities = exp_values / np.sum(exp_values)
        return np.asarray(probabilities)

    def _softmax_eps(self) -> None:
        """
        Softmax epsilon update rule.
        """
        raise NotImplementedError

    def _softmax_alpha(self) -> None:
        """
        Softmax alpha update rule.
        """
        raise NotImplementedError

    def _reward_based_eps(self) -> None:
        """
    ...
    def _state_based_alpha(self) -> None:
        """
    ...
    def update_timestep_data(
        ...
        if self.reward_list is not None and self.current_reward is not None:
            if len(self.reward_list) == 2:
                # Moves old current reward to now last reward, current reward first
    ...
    def update_eps(self) -> None:
        ...
        if self.epsilon_strategy in self.epsilon_strategies:
            ...
            raise HyperparameterError(
                ...
                f"Please check your configuration and specify a valid epsilon strategy."
            )
    ...
    def update_alpha(self) -> None:
        ...
        if self.alpha_strategy in self.alpha_strategies:
            ...
            raise HyperparameterError(
                ...
                f"Please check your configuration and specify a valid alpha strategy."
            )
...
def _a2c_hyperparams(sim_dict: dict[str, Any], trial: Trial) -> dict[str, Any]:
    params: dict[str, Any] = {}
    params["normalize"] = trial.suggest_categorical("normalize", [True, False])
    params["n_timesteps"] = sim_dict["num_requests"] * sim_dict["max_iters"]
    params["policy"] = trial.suggest_categorical(
...
def _dqn_hyperparams(sim_dict: dict[str, Any], trial: Trial) -> dict[str, Any]:
    params: dict[str, Any] = {}
    params["normalize"] = trial.suggest_categorical("normalize", [True, False])
    params["n_timesteps"] = sim_dict["num_requests"] * sim_dict["max_iters"]
    params["policy"] = trial.suggest_categorical(
...
def _qr_dqn_hyperparams(sim_dict: dict[str, Any], trial: Trial) -> dict[str, Any]:
...
def _drl_hyperparams(sim_dict: dict[str, Any], trial: Trial) -> dict[str, Any]:
