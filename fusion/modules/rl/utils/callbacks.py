...
class EpisodicRewardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if done:
            self.episode_rewards = np.append(
                self.episode_rewards, self.current_episode_reward
            )
            if ((self.iteration % self.sim_dict["save_step"]) == 0) or (
            ...
            if self.verbose:
                print(
                    f"Episode {len(self.episode_rewards)} finished with reward: {self.current_episode_reward}"
                )
    ...
    def _on_step(self) -> bool:
        ...
        if done:
            ...
            if self.verbose > 0:
                if self.sim_dict["path_algorithm"] in ("ppo", "a2c"):
                    print(
                        f"[LearnRateEntCallback] Episode {self.iteration} finished. "
