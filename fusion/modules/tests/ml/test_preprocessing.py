...
class TestProcessTrainingData:
    ...
    def test_process_with_valid_data_returns_dataframe(
        self, mock_plot: Mock
    ) -> None:
    ...
    @patch('fusion.modules.ml.preprocessing.plot_data_distributions')
    def test_process_creates_one_hot_encoded_bandwidth(
        self, mock_plot: Mock
    ) -> None:
        ...
        sim_dict = {'train_file_path': 'test_experiment'}
        df = pd.DataFrame({
            'path_length': [10, 20, 30],
            'old_bandwidth': [50, 100, 50]
        })
        erlang = 1000.0
    ...
    @patch('fusion.modules.ml.preprocessing.plot_data_distributions')
    def test_process_with_empty_dataframe_returns_empty(
        self, mock_plot: Mock
    ) -> None:
...
class TestBalanceTrainingData:
    ...
    @patch('fusion.modules.ml.preprocessing.process_training_data')
    def test_balance_equally_samples_minimum_size(
        self, mock_process: Mock
    ) -> None:
        """Test equal balancing samples minimum class size."""
        # Arrange
        df = pd.DataFrame({
            'num_segments': [1, 1, 1, 2, 2, 4, 4, 4, 4]
        })
        mock_process.return_value = df
        sim_dict = {'train_file_path': 'test'}
        erlang = 1000.0
    ...
    @patch('fusion.modules.ml.preprocessing.process_training_data')
    def test_balance_weighted_applies_weights(
        self, mock_process: Mock
    ) -> None:
