...
def sdn_props(topology: nx.Graph) -> Mock:
    ...
    props.network_spectrum_dict = {
        ("A", "B"): {"cores_matrix": {"c": [np.zeros(320) for _ in range(7)]}},
        ("B", "C"): {"cores_matrix": {"c": [np.ones(320) for _ in range(7)]}},
        ("A", "C"): {"cores_matrix": {"c": [np.zeros(320) for _ in range(7)]}},
    }
    props.mod_formats = {
        "QPSK": {"max_length": 1000, "slots_needed": 10},
        "16-QAM": {"max_length": 500, "slots_needed": 8},
...
@pytest.fixture
def nli_aware(engine_props: dict[str, Any], sdn_props: Mock) -> Any:
...
class TestNLIAwareRouting:
    ...
    def test_get_paths_returns_paths_ordered_by_nli(self, nli_aware: Any) -> None:
    ...
    def test_get_metrics_returns_algorithm_statistics(self, nli_aware: Any) -> None:
    ...
    def test_calculate_path_nli_returns_valid_metric(self, nli_aware: Any) -> None:
    ...
    def test_calculate_path_nli_with_empty_path(self, nli_aware: Any) -> None:
    ...
    def test_find_least_weight_populates_route_props(self, nli_aware: Any) -> None:
