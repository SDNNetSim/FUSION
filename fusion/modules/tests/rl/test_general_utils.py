"""Unit tests for fusion.modules.rl.utils/general_utils module."""
...
def _engine(snapshot: bool = True) -> SimpleNamespace:
    return SimpleNamespace(
        engine_props={
            "snapshot_step": 1,
...
class TestUpdateSnapshots:
    ...
    def test_calls_update_when_condition_met(self) -> None:
...
class TestClassifyPathsAndCores:
    ...
    def test_classify_paths_returns_info(self, *_: mock.MagicMock) -> None:
        """Returns list of tuples (idx,path,cong)."""
        helper = gu.CoreUtilHelpers(_rl_props(), _engine(), _route())
        paths = np.array([[[0, 1]], [[0, 2]]])
        info = helper.classify_paths(paths)

        assert info[0][0] == 0  # index
        assert info[0][2] == 2  # congestion level
        assert list(info[0][1]) == [0, 1]  # path list
...
class TestAllocateBlocking:
    """allocate branch when chosen index unavailable."""

    def test_blocks_and_updates_stats(self) -> None:
...
class TestDetermineModelType:
    """determine_model_type returns correct key."""

    def test_returns_path_algorithm(self) -> None:
        """Detects path_algorithm key."""
        sim = {"path_algorithm": "ppo"}
        assert gu.determine_model_type(sim) == "path_algorithm"

    def test_raises_when_missing(self) -> None:
        """Raises ValueError when no algo keys present."""
        with pytest.raises(gu.ConfigurationError):
