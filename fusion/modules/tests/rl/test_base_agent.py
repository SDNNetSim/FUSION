...
"""Unit tests for fusion.modules.rl.agents.base_agent module."""
...
class TestBaseAgent:
    ...
    # ------- helpers ---------------------------------------------------
    def _minimal_engine_props(self) -> dict[str, Any]:
    ...
    # ------- calculate_* ----------------------------------------------
    def test_calculate_dynamic_penalty_value(self, mock_hpc: mock.MagicMock) -> None:
        """Dynamic penalty equals core_index × beta."""
        agent = self._new_agent(mock_hpc=mock_hpc)
        assert agent.calculate_dynamic_penalty(4, 2) == 4.0

    def test_calculate_dynamic_reward_value(self, mock_hpc: mock.MagicMock) -> None:
        """Dynamic reward decays per req_id."""
        agent = self._new_agent(mock_hpc=mock_hpc)
        assert agent.calculate_dynamic_reward(2, 10) == pytest.approx(4.5)

    # ------- get_reward -----------------------------------------------
    def test_get_reward_static_allocated(self, mock_hpc: mock.MagicMock) -> None:
    ...
    def test_get_reward_static_not_allocated(self, mock_hpc: mock.MagicMock) -> None:
    ...
    def test_setup_env_bad_algorithm_raises(self, mock_hpc: mock.MagicMock) -> None:
        ...
        agent = self._new_agent("unknown_algo", mock_hpc=mock_hpc)
