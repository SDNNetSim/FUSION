...
"""Unit tests for fusion.modules.rl.agents.path_agent module."""
...
class TestPathAgent:
    ...
    # ------------------------------------------------------------------
    def _mk_agent(self, alg: str = "q_learning") -> path_agent.PathAgent:
        ...
        agent.reward_penalty_list = np.array([0.0] * 10)
    ...
    def test_end_iter_updates_hyperparams(self) -> None:
        """end_iter increments iteration and updates α, ε."""
        agent = self._mk_agent()
        agent.end_iter()
        assert agent.hyperparam_obj is not None
        assert agent.hyperparam_obj.iteration == 1
        agent.hyperparam_obj.update_alpha.assert_called_once()
        agent.hyperparam_obj.update_eps.assert_called_once()

    @mock.patch.object(path_agent, "EPISODIC_STRATEGIES", ["episodic"])
    def test_end_iter_skips_updates_for_bandit_ucb(self) -> None:
    ...
    # ----------------------- _handle_hyperparams ----------------------
    @mock.patch.object(path_agent, "EPISODIC_STRATEGIES", ["episodic"])
    def test_handle_hyperparams_updates_state_and_rates(self) -> None:
    ...
    # ----------------------------- update -----------------------------
    def test_update_raises_at_max_iters(self) -> None:
    ...
    def test_update_returns_early_for_drl(self) -> None:
    ...
    # ------------------------- _bandit_route --------------------------
    def test_bandit_route_selects_arm_and_sets_attrs(self) -> None:
        """_bandit_route sets chosen indices for bandit agent."""
        agent = self._mk_agent(alg="epsilon_greedy_bandit")
        assert agent.hyperparam_obj is not None
        assert agent.algorithm_obj is not None
