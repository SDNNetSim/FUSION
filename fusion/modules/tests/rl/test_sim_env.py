"""Unit tests for fusion.modules.rl.utils.sim_env module (helpers only)."""
...
def _make_sim_env(
    ...
    return SimpleNamespace(
        ...
        # Some classes expect engine_props instead of engine_obj
        engine_props=engine_obj,
...
class TestCheckTerminated:
    ...
    def test_increments_iter_and_calls_end(self, _: mock.MagicMock) -> None:
        ...
        assert senv.iteration == 1
        senv.path_agent.end_iter.assert_called_once()
        senv.engine_obj.end_iter.assert_called_once()


# ------------------------------------------------------------------ #
class TestScaleReqHolding:
    """SimEnvObs._scale_req_holding numeric scaling."""

    @pytest.fixture
    def obs(self) -> SimEnvObs:
        """Create SimEnvObs instance."""
        senv = _make_sim_env()
        return SimEnvObs(senv)

    def test_correct_scaling_value(self, obs: SimEnvObs) -> None:
...
# ------------------------------------------------------------------ #
class TestUpdateHelperObj:
    """SimEnvObs.update_helper_obj path-index assignment."""

    def test_sets_path_index_for_drl_agent(self) -> None:
        ...
        assert senv.rl_help_obj.path_index == 2
        assert senv.rl_help_obj.rl_props.forced_index is None
